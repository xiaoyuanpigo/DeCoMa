    ...
    with open(filename, encoding="utf-8") as f:
        for idx, line in enumerate(f):
            ...
            if 'idx' not in js:
                js['idx'] = idx
            code = js['code']
            nl = js['comment'] if 'comment' in js.keys() else " ".join(js['docstring_tokens'])
            examples.append(
                Example(
                    idx=idx,
                    source=code,
...
def main():
    ...
    model = Seq2Seq(model=encoder, config=config)
        ...
        for epoch in range(args.num_train_epochs):
            ...
            if args.do_eval:
                ...
                for batch in eval_dataloader:
                    ...
                    with torch.no_grad():
                        preds = model.generate(source_ids, attention_mask=source_mask, use_cache=True,
                                               num_beams=args.beam_size, early_stopping=True,
